    ...
    public func transcribe(samples: [Float], vocabulary: [String] = [],
        ...
        prompt.withCString { pptr in
            "en".withCString { lptr in
                ...
                // No timestamps of any kind. They were carried for one
                // purpose — measuring the pauses between words, to break
                // paragraphs on them — and that whole idea is gone: six
                // attempts, and the best of them still split "rounds out the /
                // Paragraph" in half, because a speaker drawing breath
                // mid-sentence is indistinguishable from one starting a new
                // thought. Paragraphs come from the polish model or from the
                // words "new paragraph", both of which know what the sentence
                // means.
