use of_formatter_spike::*;

fn f(s: &str) -> String { format(s, &Config::default()) }
fn v(inp: &str, out: &str) -> Verdict { check(inp, out, &Config::default()) }
...
fn keeps_words_that_only_look_like_fillers() {
    ...
    assert_eq!(f("Her answer was ah so clever."), "Her answer was ah so clever.");
...
fn structure_commands_fire() {
    assert_eq!(f("ship it new paragraph then tell the team"),
               "Ship it\n\nThen tell the team");
...
fn punctuation_commands_off_by_default() {
    ...
    assert_eq!(f("the period after ovulation"), "The period after ovulation");
...
fn catches_invented_text() {
    let r = v("the deploy failed", "The deploy failed. Let me know if you need anything else!");
    match r {
...
fn catches_a_helpful_rewrite() {
    ...
    let r = v("i seen the logs and they was fine", "I saw the logs and they were fine.");
    assert!(matches!(r, Verdict::Fail { .. }), "guardrail let a rewrite through");
}

#[test]
fn tolerates_pure_formatting() {
    assert_eq!(v("first do this second do that",
                 "1. do this\n2. do that"), Verdict::Pass);
...
fn failed_stage_falls_through_unchanged() {
    ...
    let (out, verdict) = run_stage("the deploy failed", &cfg, |_| "Something else entirely".into());
...
fn command_between_sentences_leaves_no_stray_punctuation() {
    ...
    assert_eq!(v("the config map wasn't updated. New paragraph. Let's verify the health checks.", &out),
               Verdict::Pass);
}

// ---- tone ---------------------------------------------------------------

fn t(s: &str, tone: Tone) -> String { apply_tone(&f(s), tone) }
...
fn casual_keeps_capitals_and_breaks_lines() {
    let out = t("I'm running late. I'll be there in ten minutes, sorry.", Tone::Casual);
    assert_eq!(out, "I'm running late\nI'll be there in ten minutes, sorry");
}

#[test]
fn very_casual_lowercases_and_drops_commas() {
    let out = t("I'm running late. I'll be there in ten minutes, sorry.", Tone::VeryCasual);
    assert_eq!(out, "i'm running late\ni'll be there in ten minutes sorry");
}
...
fn question_and_exclamation_survive_every_tone() {
    ...
    for tone in [Tone::Casual, Tone::VeryCasual] {
        ...
        assert!(out.contains('?') && out.contains('!'), "{tone:?} -> {out:?}");
    }
}

#[test]
fn decimals_and_abbreviations_are_not_split() {
    let out = t("The incident started at 2.15 in the afternoon.", Tone::Casual);
...
fn tone_is_word_preserving_in_every_register() {
    ...
    for raw in [
        ...
        for tone in [Tone::Formal, Tone::Casual, Tone::VeryCasual] {
            ...
            assert_eq!(check(raw, &out, &cfg), Verdict::Pass, "{tone:?} raw={raw:?} out={out:?}");
        }
    }
}
...
fn casual_drops_the_period_before_an_existing_break() {
    ...
    let out = t("the config map wasn't updated. New paragraph. Let's verify.", Tone::Casual);
    assert!(!out.contains(".\n"), "stranded period: {out:?}");
...
fn span_stops_at_the_end_of_the_noun_phrase() {
    ...
    assert_eq!(f("The quote-unquote experts said it would never work."),
               "The \"experts\" said it would never work.");
    assert_eq!(f("That is quote-unquote best practice according to them."),
               "That is \"best practice\" according to them.");
}

#[test]
fn explicit_closer_gives_an_exact_span() {
    assert_eq!(f("She said quote this is the best release we have ever shipped unquote and then left."),
               "She said \"this is the best release we have ever shipped\" and then left.");
    assert_eq!(f("He called it open quote a rounding error close quote which was generous."),
...
fn the_ordinary_noun_quote_is_left_alone() {
    ...
    assert_eq!(f("She gave me a quote for the work last Tuesday."),
...
fn quoting_passes_the_guardrail() {
    ...
    for raw in [
        ...
        assert_eq!(check(raw, &out, &cfg), Verdict::Pass, "raw={raw:?} out={out:?}");
    }
}
...
fn a_declared_vocabulary_fix_is_allowed() {
    let cfg = Config::default();
    let edits = [Edit { from: "Largemont".into(), to: "Larchmont".into(),
                        reason: EditReason::Vocabulary }];
    assert_eq!(
        check_declared("we should meet in Largemont at noon",
                       "We should meet in Larchmont at noon.",
                       &edits, &Policy::default(), &cfg),
        EditVerdict::Pass);
}
...
fn the_same_fix_undeclared_is_still_caught() {
    let cfg = Config::default();
    match check_declared("we should meet in Largemont at noon",
                         "We should meet in Larchmont at noon.",
                         &[], &Policy::default(), &cfg) {
...
fn a_category_the_operator_disallowed_is_refused() {
    let cfg = Config::default();
    let edits = [Edit { from: "i seen".into(), to: "I saw".into(),
                        reason: EditReason::Other }];
    assert_eq!(
        check_declared("i seen the logs", "I saw the logs.",
                       &edits, &Policy::default(), &cfg),
...
fn self_correction_becomes_expressible() {
    ...
    let cfg = Config::default();
    let edits = [Edit { from: "Bob I mean Bill".into(), to: "Bill".into(),
                        reason: EditReason::SelfCorrection }];
    assert_eq!(
        check_declared("send it to Bob I mean Bill on Friday",
                       "Send it to Bill on Friday.",
                       &edits,
                       &Policy { allowed: vec![EditReason::SelfCorrection],
                                 ..Policy::default() }, &cfg),
        EditVerdict::Pass);
}
...
fn empty_declaration_is_the_old_exact_equality_check() {
    let cfg = Config::default();
    assert_eq!(check_declared("the deploy failed", "The deploy failed.",
                              &[], &Policy::strict(), &cfg), EditVerdict::Pass);
}
...
fn a_stage_may_fix_a_word_but_not_reword_you() {
    ...
    let one = [Edit { from: "Largemont".into(), to: "Larchmont".into(),
                      reason: EditReason::Vocabulary }];
    assert_eq!(check_declared(
        ...
        &one, &Policy::default(), &cfg), EditVerdict::Pass);
    ...
    // percentage.
    assert_eq!(check_declared("meet in Largemont today", "Meet in Larchmont today.",
                              &one, &Policy::default(), &cfg), EditVerdict::Pass);

    // but rewording is refused: five of nine words changed, all declared
    let many: Vec<Edit> = [("we","they"),("should","will"),("meet","gather")].iter()
        .map(|(f,t)| Edit { from: (*f).into(), to: (*t).into(),
                            reason: EditReason::Vocabulary }).collect();
    match check_declared("we should meet in Largemont at noon",
                         "They will gather in Largemont at noon.",
                         &many, &Policy { max_edits: 9, ..Policy::default() }, &cfg) {
...
fn too_many_edits_is_refused_even_when_each_is_allowed() {
    let cfg = Config::default();
    let many: Vec<Edit> = ["a", "b", "c", "d"].iter()
        .map(|w| Edit { from: (*w).into(), to: "x".into(), reason: EditReason::Vocabulary })
        .collect();
    match check_declared("a b c d and some more words here to pad it out",
                         "x x x x and some more words here to pad it out",
                         &many, &Policy::default(), &cfg) {
...
fn fe(s: &str) -> (String, Vec<Edit>) { format_with_edits(s, &Config::default()) }

#[test]
fn a_false_start_is_erased_and_declared() {
    let (out, edits) = fe("Hey buddy, I wanted to say thank- no, no, no. I wanted to thank you for helping me out.");
...
fn corrections_are_accounted_for_by_the_ledger() {
    let cfg = Config::default();
    let raw = "Hey buddy, I wanted to say thank- no, no, no. I wanted to thank you for helping me out.";
    let (out, edits) = fe(raw);
    assert_eq!(check_declared(raw, &out, &edits, &Policy::default(), &cfg), EditVerdict::Pass);
    // ...and the same output with nothing declared is correctly rejected
    assert!(matches!(check_declared(raw, &out, &[], &Policy::default(), &cfg),
...
fn the_list_connector_and_is_declared_not_smuggled() {
    ...
    assert!(edits.iter().any(|e| e.from == "and" && e.reason == EditReason::Structure),
...
fn a_salutation_gets_its_own_block() {
    let out = apply_letter_layout(&f("Hi John, I wanted to ask about the deploy."), Tone::Formal);
